...
const CompactSegmentedControl = ({
  ...
  return (
    <View className="mx-12 mb-4 mt-4 h-10 bg-white/[0.03] rounded-full border border-white/[0.1] p-1 flex-row relative">
      <TouchableOpacity
      ...
      >
        <Ionicons name="earth" size={12} color={selected === "global" ? "black" : "#666"} style={{ marginRight: 4 }} />
        <Text
          className={`text-[11px] font-black tracking-wide ${selected === "global" ? "text-black" : "text-white/40"}`}
        >
          GLOBAL
        </Text>
      </TouchableOpacity>
...
export default function Ranking() {
  ...
  const currentLeaderboard = activeTab === "global" ? leaderboard?.global : leaderboard?.friends;

  const entries = useMemo(() => currentLeaderboard?.entries || [], [currentLeaderboard]);
      ...
      <View className="flex-row justify-center items-end mb-10 mt-6 px-4 gap-3">
        {second && (
          <View className="items-center w-1/3 mb-4">
            ...
            <Text className="text-white/40 text-xs font-bold">{second.alcoholism_coefficient.toFixed(0)} PTS</Text>
        ...
        {first && (
          <View className="items-center w-1/3 z-10">
            ...
            <Text className="text-white/60 text-xs font-bold">{first.alcoholism_coefficient.toFixed(0)} PTS</Text>
        ...
        {third && (
          <View className="items-center w-1/3 mb-2">
            ...
            <Text className="text-white/40 text-xs font-bold">{third.alcoholism_coefficient.toFixed(0)} PTS</Text>
    ...
    return (
      <TouchableOpacity
        ...
        className={`flex-row items-center p-4 mx-4 mb-3 rounded-2xl border ${
          isMe ? "bg-orange-600/10 border-orange-600" : "bg-white/[0.03] border-white/[0.08]"
        }`}
      >
        <Text className={`text-xl font-black w-10 text-center mr-2 ${isMe ? "text-orange-500" : "text-white/30"}`}>
          {item.rank}
        </Text>

        <View className="w-12 h-12 rounded-full overflow-hidden mr-4 bg-[#1A1A1A] border border-white/10">
          <Image source={{ uri: item.image_url || "https://github.com/shadcn.png" }} className="w-full h-full" />
        </View>

        <View className="flex-1">
          <View className="flex-row items-center gap-2">
            <Text className={`font-bold text-base ${isMe ? "text-orange-500" : "text-white"}`}>{item.username}</Text>
            {isMe && (
        ...
        <View className="items-end">
          <Text className="text-orange-600 font-black text-xl">{item.alcoholism_coefficient.toFixed(0)}</Text>
  ...
  const renderListHeader = () => {
    return (
      <View>
        <View className="px-4 z-10 bg-black">
  ...
  return (
    <View className="flex-1 bg-black">
      ...
      <View className="flex-1" style={{ paddingTop: insets.top }}>
        <NestedScreenHeader eyebrow="BIGGEST ALCOHOLIC" title="Ranking" showBack={true} />
      ...
      {userPosition && (
        <View
          className="absolute bottom-0 left-0 right-0 bg-black border-t-2 border-orange-600 pt-4 px-4 shadow-2xl"
          style={{ paddingBottom: insets.bottom + 10 }}
        >
          <View className="flex-row items-center justify-between">
            <View className="flex-row items-center flex-1">
              <View className="items-center mr-4 w-8">
                <Text className="text-white font-black text-xl leading-5">#{userPosition.rank}</Text>
              </View>

              <Image
                source={{
                  uri: userPosition.image_url || "https://github.com/shadcn.png",
                }}
            ...
            <View className="items-center bg-orange-600/10 px-4 py-2 rounded-xl border border-orange-600/30">
              ...
              <Text className="text-orange-500/50 text-[8px] font-bold uppercase text-right">Points</Text>
            </View>
          </View>
        </View>
      )}
