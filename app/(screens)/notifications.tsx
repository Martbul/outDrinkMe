  ...
  const renderNotification = (notification: NotificationItem) => {
    ...
    return (
      <TouchableOpacity
        key={notification.id}
        onPress={() => {
          // --- DEBUGGING: Check your logs to see what keys are actually available ---
          console.log(
            "Notification payload:",
            JSON.stringify(safeData, null, 2)
          );

          // 1. Aggressively look for the ID
          // We check multiple common keys your backend might be sending
          const postIdToOpen = safeData.post_id;

          // 2. Perform Navigation FIRST (before the component potentially unmounts due to filter)
          if (postIdToOpen) {
            console.log("Redirecting to Post ID:", postIdToOpen);
            router.push({
              pathname: "/(tabs)/mix", // Ensure this path matches your file structure exactly
              params: { openPostId: postIdToOpen },
            });
          } else {
            console.warn("No ID found in notification data. Cannot redirect.");
          }

          // 3. Mark as read AFTER triggering navigation
          if (isUnread) {
            markNotificationRead(notification.id);
          }
        }}
