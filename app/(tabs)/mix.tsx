...
import { RefreshControl } from "react-native";
import { useSafeAreaInsets } from "react-native-safe-area-context";
import { VideoPost } from "@/components/mixVideo"; // Removed unused MixVideo import
import { apiService } from "@/api";
import { useAuth } from "@clerk/clerk-expo";
import { usePostHog } from "posthog-react-native";
...
// --- HELPER: Optimize Cloudinary URLs for display ---
const getOptimizedImageUrl = (url: string, width = 800) => {
  ...
  // w_800: Resize to width
  // c_fill,g_auto: Crop to fill and focus on the most interesting part (faces/objects)
...
const MixScreen = () => {
  ...
  const flatListRef = useRef<FlatList>(null);
  const insets = useSafeAreaInsets();
  // const [videos, setVideos] = useState<VideoPost[]>([]); // Commented out as per your code
  // const [showRecordModal, setShowRecordModal] = useState(false); // Commented out
  ...
  const YourMixCard = ({ item }: { item: YourMixPostData }) => {
    ...
    // Use optimized URL for the feed to prevent memory crashes
    ...
    const renderContent = () => {
      if (flipState === 0) {
        return (
          <>
            {/* 
               Using the optimized URL + resizeMode="cover" ensures the image
               fills the 4:3 card completely without distortion.
               'g_auto' in the URL handles smart cropping if aspect ratios differ.
            */}
            <Image
