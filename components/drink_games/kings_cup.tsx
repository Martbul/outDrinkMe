      ...
      <View className="flex-1 bg-black/80 justify-end">
        <View className="bg-[#1a1a1a] rounded-t-3xl border-t border-white/10 p-6 h-[60%]">
          ...
          <Text className="text-white/70 text-base text-center mb-6">
            When you drink, your buddy drinks.
          </Text>
        ...
        {!currentCard ? (
          <TouchableOpacity
            ...
            className={`w-[75%] aspect-[2.5/3.5] bg-orange-600 rounded-2xl border-4 border-white/10 items-center justify-center shadow-xl ${
              isMyTurn ? "active:bg-orange-700" : "opacity-50"
            }`}
          >
            <MaterialCommunityIcons
            ...
            >
              {Object.entries(state.buddies || {}).map(
                ([playerID, buddyList]) => {
                  ...
                  if (!p || !buddyList.length) return null;

                  return buddyList.map((buddy, index) => (
                    <View
                      key={`buddy-${playerID}-${index}`}
                      className="flex-row items-center bg-[#ff8c00]/10 border border-[#ff8c00] rounded-xl px-3 py-3 h-14"
                    >
                      <View className="items-center">
                        <Text
                          className="text-[#ff8c00] font-bold text-xs max-w-[60px]"
                          numberOfLines={1}
                        >
                          {p.username}
                        </Text>
