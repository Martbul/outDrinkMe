...
interface ImagePickerModalProps {
  visible: boolean;
  onClose: () => void; 
  onSelectCamera: () => void | Promise<void>;
  onSelectLibrary: () => void | Promise<void>; 
}
...
export const ImagePickerModal = ({
  ...
  return (
    <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
      <TouchableOpacity className="flex-1 bg-black/70 items-center justify-center" activeOpacity={1} onPress={onClose}>
        <View className="bg-black rounded-2xl p-5 mx-4 w-[90%] max-w-md border border-white/[0.08]">
          <Text className="text-orange-600 text-xl font-black mb-4">Choose Image Source</Text>

          <TouchableOpacity
            onPress={onSelectCamera}
            className="flex-row items-center bg-white/[0.03] rounded-xl p-4 mb-3 border border-white/[0.08]"
            activeOpacity={0.7}
          >
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.7}
          >
            <Text className="text-white/50 text-center font-semibold">Cancel</Text>
          </TouchableOpacity>
        </View>
      </TouchableOpacity>
    </Modal>
