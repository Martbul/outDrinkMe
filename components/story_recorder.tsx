...
import { Ionicons } from "@expo/vector-icons";
// FIX 1: Import useMicrophonePermissions
import {
  CameraView,
...
export function StoryRecorder({ onClose }: { onClose: () => void }) {
  // FIX 2: Initialize both permission hooks
  ...
  if (!cameraPermission.granted || !microphonePermission.granted) {
    ...
    return (
      <View className="flex-1 bg-black justify-center items-center px-6">
        <Text className="text-white mb-2 font-bold text-lg">
          Permissions Required
        </Text>
        <Text className="text-white/60 mb-6 text-center">
          To record a story, we need access to your Camera and Microphone.
        </Text>
        <TouchableOpacity
        ...
        <TouchableOpacity onPress={onClose} className="mt-8">
          <Text className="text-white/50 font-bold">Cancel</Text>
        </TouchableOpacity>
      </View>
    );
  }
    ...
    if (cameraRef.current) {
      ...
      try {
        const video = await cameraRef.current.recordAsync({
          maxDuration: 15,
          quality: "1080p",
          // mute: false is default, which triggers the need for AUDIO permission
        });
        if (video) setVideoUri(video.uri);
      } catch (e) {
        ...
        Alert.alert(
          "Error",
          "Could not record video. Please check permissions."
        );
      } finally {
        setIsRecording(false);
      }
