...
// export function StoryRecorder({ onClose,showFeedback }: { onClose: () => void }) {
...
// 1. Define the props interface properly
type FeedbackType = "success" | "xp" | "level";
...
export function StoryRecorder({ onClose, showFeedback }: StoryRecorderProps) {
  const [cameraPermission, requestCameraPermission] = useCameraPermissions();
  const [microphonePermission, requestMicrophonePermission] =
    useMicrophonePermissions();
  ...
  const handlePostStory = async () => {
    ...
    try {
      ...
      showFeedback("Uploading story! +100 XP", "xp");
