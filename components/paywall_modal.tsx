...
import { PaddleCheckout } from "./paddle_checkout"; // Import the component we made above
...
export const PaywallModal = ({ visible, onClose, onPurchase }: PaywallSheetProps) => {
  ...
  const [showCheckout, setShowCheckout] = useState(false); // New state to toggle views
  ...
  const handleCheckoutSuccess = () => {
    // 1. Close the checkout view
    setShowCheckout(false);
    // 2. Trigger parent logic (e.g., update context to true)
    onPurchase();
    // 3. Close the entire modal
    onClose();
  };
  ...
  const FeatureItem = ({ icon, title, subtitle }: { icon: string; title: string; subtitle: string }) => (
    <View className="flex-row items-center mb-5 last:mb-0">
      ...
      <View className="flex-1">
        ...
        <Text className="text-white/50 text-[11px] font-semibold mt-0.5">{subtitle}</Text>
      </View>
    </View>
  );
  ...
  return (
    ...
    >
      <View className="pb-10 min-h-[600px]">
        {/* CONDITIONAL RENDERING: Checkout vs Sales Page */}
        {showCheckout ? (
          <PaddleCheckout
            plan={billing}
            onSuccess={handleCheckoutSuccess}
            onClose={() => setShowCheckout(false)} // Go back to pricing
          />
        ) : (
          /* SALES PAGE CONTENT */
          <>
            ...
            <View className="mb-8">
              <Text className="text-white/50 text-[11px] font-bold tracking-widest mb-4 ml-1">CHOOSE YOUR PLAN</Text>

              <View className="flex-row gap-3">
                <Pressable
                  onPress={() => setBilling("yearly")}
                  className={`flex-1 p-4 rounded-2xl border-2 relative ${
                    billing === "yearly" ? "bg-[#1A1A1A] border-orange-600" : "bg-white/[0.02] border-white/[0.08]"
                  }`}
                >
                  {billing === "yearly" && (
                  ...
                  <View className="items-center mt-2">
                    ...
                    <Text className="text-white text-2xl font-black">{pricing.yearly}</Text>
                    <Text className="text-white/30 text-[9px] font-bold mt-1">Billed every 12 months</Text>
                  </View>
                </Pressable>
                ...
                <Pressable
                  ...
                  className={`flex-1 p-4 rounded-2xl border-2 relative ${
                    billing === "monthly" ? "bg-[#1A1A1A] border-orange-600" : "bg-white/[0.02] border-white/[0.08]"
                  }`}
                >
                  <View className="items-center mt-2">
                    <Text
                    ...
                    <Text className="text-white text-2xl font-black">{pricing.monthly}</Text>
                    <Text className="text-white/30 text-[9px] font-bold mt-1">Billed monthly</Text>
                  </View>
                </Pressable>
              </View>
            </View>

            {/* BUTTON TRIGGERS CHECKOUT VIEW */}
            <TouchableOpacity
              onPress={() => setShowCheckout(true)}
              activeOpacity={0.8}
              className="bg-orange-600 py-4 rounded-2xl items-center mb-6 shadow-lg shadow-orange-600/20 border-t border-white/20"
            >
              <Text className="text-black font-black text-base tracking-widest uppercase">Start {billing} Plan</Text>
